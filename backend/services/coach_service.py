...
def _generate_human_explanation(bmi: float, level: str, goal: str) -> str:
    ...
    parts = [f"Your current BMI is {bmi_round}, which places you in the **{level}** category."]
    
    if bmi < 18.5:
        parts.append("Being underweight reduces muscle mass and immune function. A caloric surplus with high-protein foods is critical.")
    elif bmi < 25:
        parts.append("Your weight is in a healthy range! Maintaining this balance through consistent activity and nutrition is the key.")
    elif bmi < 30:
        parts.append("Being overweight slightly elevates your risk for cardiovascular issues. A moderate caloric deficit will yield excellent results.")
    else:
        parts.append("Obesity significantly raises the risk of diabetes and heart disease. Start with low-impact cardio and portion control for safe fat loss.")
        
    parts.append(f"Since your specific goal is **{goal}**, I have designed a personalized action plan below to help you achieve it.")
    return " ".join(parts)
...
class CoachService:
    def get_coach_advice(self, bmi, fitness_level, food_pref, goal):
        ...
        try:
            ...
            rag_context = "\n".join([f"- {fact}" for fact in rag_hints]) if rag_hints else "Hydration is key."
            print(f"📚 [RAG] Successfully retrieved {len(rag_hints)} facts.")
        except Exception as e:
            print(f"⚠️ [RAG] System failed gracefully ({e}). Falling back without context.")
        ...
        if llm:
            try:
                ...
                print(f"❌ [LLM] OpenAI Error: {e}. Switching to Smart Fallback System.")
                
        # 4. IF NO API KEY OR LLM CRASHES -> SMART RANDOMIZED FALLBACK
        print("🔄 [FALLBACK] Generating randomized smart fallback plan.")
        return self._rule_based_fallback(fitness_level, food_pref, bmi, goal, explanation)
        ...
        for key in ["DIET", "YOGA", "EXERCISE", "SUMMARY"]:
            ...
            if match:
                ...
                if key in ["YOGA", "EXERCISE", "DIET"]:
                     ...
                     clean = [item.strip() for item in re.split(r',|\n-', content.replace("[", "").replace("]", "")) if item.strip()]
    ...
    def _rule_based_fallback(self, level, pref, bmi, goal, explanation):
        ...
        diets = {
            "veg_loss": ["Lentil Soup & Cucumber Salad", "Quinoa Veggie Bowl", "Green Tea & Sprout Salad", "Brown Rice & Moong Dal", "Oats with chia seeds"],
            "nonveg_loss": ["Grilled Chicken & Broccoli", "Baked Salmon", "Chicken Salad with Olive Oil", "Boiled Eggs & Spinach", "Turkey Lettuce Wraps"],
            "veg_gain": ["Tofu & Sweet Potato", "Soy Chunks Pulav", "Paneer Tikka Meal", "Chickpea Curry", "Peanut Butter Banana Shake"],
            "nonveg_gain": ["Chicken Breast & Quinoa", "Steak & Roasted Potatoes", "Eggs & Oatmeal", "Turkey Pasta", "Greek Yogurt & Mince Meat"],
            "veg_maint": ["Dal & Brown Rice", "Khichdi", "Mixed Veg Curry", "Oats & Fruit Bowl", "Veggie Wrap"],
            "nonveg_maint": ["Chicken Curry & Rice", "Fish Tikka", "Egg Curry", "Grilled Chicken Wrap", "Tuna Salad"]
        }
        
        workouts = {
            "loss": ["30 min Brisk Walk", "15 min HIIT", "Stair Climbing (10 min)", "Jump Rope (5 min)", "Cycling at moderate pace"],
            "gain": ["Progressive Strength Training", "Heavy Weightlifting (Squats/Deadlifts)", "Resistance Band Exercises", "Pullups & Pushups"],
            "maint": ["20 min Morning Jog", "Bodyweight Squats (20 reps)", "Core Planks (3 sets)", "Swimming (20 min)", "Light Callisthenics"]
        }
        
        yogas = ["Surya Namaskar (10 rounds)", "Balasana (Child Pose)", "Trikonasana (Triangle Pose)", "Bhujangasana (Cobra)", "Vrikshasana (Tree Pose)"]
