...
async def heart_health_check(req: HeartRequest):
    try:
        ...
        bp_status = "safe"
        if req.blood_pressure >= 140:
            bp_status = "high"
        elif req.blood_pressure >= 130:
            bp_status = "moderate"
        ...
        if risk_final == "Critical":
            ...
            risk_desc = "Moderate"
            coach_tone = (
                "You’re close to a risk zone. Small changes can make a big difference."
            )
            warning_msg = None
        else:
            risk_desc = "Low"
        ...
        raise HTTPException(
            status_code=500, detail="Server temporarily unavailable. Please try again."
        )
